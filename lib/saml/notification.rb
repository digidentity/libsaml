module Saml
  module Notification
    def wrap_with_notification(method, instance_method)
      class_name = self.name.underscore.gsub('/', '_')

      wrapper = <<-RUBY
      define_method "#{method}_with_notification" do |*args|
        send("#{method}_without_notification", *args).tap do |result|
          ActiveSupport::Notifications.instrument "#{method}.#{class_name}", result
        end
      end
