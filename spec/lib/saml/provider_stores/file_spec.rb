...
describe Saml::ProviderStores::File do
  ...
  describe 'initialize' do
    it 'creates a store of providers' do
      expect(file_store.providers.values.first).to be_a(Saml::BasicProvider)
    end
  end

  describe '#find_by_entity_id' do
    it 'returns the identity_provider with entity_id' do
      expect(file_store.find_by_entity_id(idp_entity_id).type).to eq('identity_provider')
    end
    it 'returns the service_provider with entity_id' do
      expect(file_store.find_by_entity_id(sp_entity_id).type).to eq('service_provider')
    end
  end
  ...
  describe '#find_by_source_id' do
    ...
    it 'returns the identity_provider with entity_id' do
      expect(file_store.find_by_source_id(idp_source_id).type).to eq('identity_provider')
    end
    it 'returns the service_provider with entity_id' do
      expect(file_store.find_by_source_id(sp_source_id).type).to eq('service_provider')
    end
    it 'returns nil when not found' do
      expect(file_store.find_by_source_id('non-existing-sha1')).to be_nil
    end
  end
